...
class TestHost(ez_clang_api.Host):
    def addDevice(info, dev: ez_clang_api.Device):
        assert dev == testDevice, "Expected given testDevice instance"

testHost = TestHost()
stream = adafruit_metro_m0.serial.connect(info, testHost, testDevice)
assert stream.connected(), "Connection should be established"

adafruit_metro_m0.serial.setup(stream, testHost, testDevice)

# Check that connect() initialized mandatory properties
